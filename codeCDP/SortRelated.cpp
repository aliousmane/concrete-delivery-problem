...
void SortNode<NodeT, DriverT>::radixSortGreatDemand(std::vector<NodeT *> & nodes,
  ...
  int max_num_digits = (int)log10(max_demand) + 1;
  ...
  for (int i = 0; i < n; i++) {
    if (i + 1 < n && nodes[i]->demand == nodes[i + 1]->demand) {
        {
            if (nodes[i]->early_tw >= nodes[i + 1]->late_tw)
            {
                std::swap(nodes[i], nodes[i + 1]);
            }
            else if(TimeSlot(*nodes[i])== TimeSlot(*nodes[i+1])){
...
void SortNode<NodeT, DriverT>::radixSortEarlyTW(std::vector<NodeT *> &nodes,
  ...
  int max_num_digits = (int)log10(max_early_tw) + 1;
...
void SortNode<NodeT, DriverT>::radixSortLateTW(std::vector<NodeT *> &nodes,
  ...
  int max_num_digits = (int)log10(max_late_tw) + 1;
...
// nodes are already sorted, randomize the order of nodes with same demand
template <class NodeT, class DriverT>
...
template <class NodeT, class DriverT>
void SortNode<NodeT, DriverT>::radixSortMinWidthTW(std::vector<NodeT *> &nodes, double max_width) {


    std::uniform_real_distribution<> dis(0, 1);

    int max_num_digits = (int)log10(max_width) + 1;
    ...
    for (int i = 0; i < max_num_digits; ++i) {
        for (NodeT *node : nodes) {
            int bucket_idx = int(double(node->late_tw - node->early_tw) / factor) % 10;
            buckets[bucket_idx].push_back(node);
        }
    ...
    int n = nodes.size();
    for (int i = 0; i < n; i++) {
        if (i + 1 < n && (nodes[i]->late_tw -  nodes[i]->early_tw) ==  (nodes[i+1]->late_tw-  nodes[i + 1]->early_tw)) {
            // if (dis(Parameters::RANDOM_GEN) < nodes[i]->demand / (nodes[i]->demand + nodes[i +
            // 1]->demand)) 	std::swap(nodes[i], nodes[i + 1]);
            if (nodes[i]->late_tw > nodes[i + 1]->late_tw)
                std::swap(nodes[i], nodes[i + 1]);
