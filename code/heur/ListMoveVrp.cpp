...
bool ListMoveVrpSorter::operator()(Move<Delivery, Driver, MoveVrp> &m1, Move<Delivery, Driver, MoveVrp> &m2) {
    ...
    if (Parameters::SORT_TYPE == Parameters::SORT::TWO) {
        return Sort2(m1, m2);
    }
    if (Parameters::SORT_TYPE == Parameters::SORT::THREE) {
        return Sort3(m1, m2);
    }
    if (Parameters::SORT_TYPE == Parameters::SORT::FOUR) {
        return Sort4(m1, m2);
    }
    if (Parameters::SORT_TYPE == Parameters::SORT::FIVE) {
        return Sort5(m1, m2);
    }
...
//    cout<<"count "<<moveMap.count(str) <<" add "<< str<<endl;
    _moves.push_back(m);
