...
class Driver {
public:
    int id,StartNodeID,EndNodeID,depotID,distID,start_shift_time,end_shift_time,capacity,serviceDuration;
    long no,rank;
    double  sumDemand,shiftDuration,shiftDurationCost;

    Driver(): id(-1), no(-1), rank(0), StartNodeID(0), EndNodeID(-1), distID(-1), depotID(-1), capacity(0),
              sumDemand(0), start_shift_time(0), end_shift_time(0),shiftDuration(0),shiftDurationCost(0),serviceDuration(0)
    {}
