...
\usepackage{multirow}
\usepackage{float}
\usepackage[text={16cm,21cm},centering]{geometry}
\usepackage{setspace}
\usepackage{array}
\usepackage{lineno}
\usepackage{breqn}
...
\affiliation[inst1]{organization={CIRRELT, Université LAVAL},%Department and Organization
...
\affiliation[inst2]{organization={Canada research chair in integrated logistics},%Department and Organization
...
The Greedy Randomized Search Procedure (GRASP) algorithm is a multi-start metaheuristic for combinatorial optimization problems proposed by \cite{feo1989probabilistic}. It has been successfully applied to solving scheduling problems \citep{bamoumen2023efficient,yepes2023flowshop} and transportation-related problems such as the prisoner transportation problem \citep{ferone2023efficient} and the truck and trailer routing problem \citep{villegas2011grasp}. The GRASP is a two-phase iterative sequence of constructive and local search algorithms. A feasible solution is generated in the construction phase using a greedy randomized algorithm. Then, a local search phase tries to improve it. Algorithm~\ref{alg_grasp} outlines the GRASP algorithm. We start with a list of greedy randomized heuristics and iterate on each to construct an incumbent solution $Sol$ on which we apply a local search procedure to explore its neighborhood. The best solution found so far is then stored. To avoid redundant computations, a hash table $T$ ensures that the local search is not applied twice to the same solution. The procedure stops and returns the best overall solution when certain stopping conditions are met, such as reaching a time limit or a maximum number of iterations.
...
\footnotesize
    \begin{algorithm}[!ht]
        ...
        $M \leftarrow \emptyset $ \small \tcp{List of pairs of loading -- unloading tasks for each delivery node}
...
$c_{max} \leftarrow \max\left\{ cost \ | \ (L^k_{d}, U^k_{d},cost,k) \in M[d]   \right\} $
    ...
    \uElse{
        ...
        \uIf{no $D^{j'}_o$ exists}{
            ...
            $Sol \leftarrow Sol \backslash \{(L^k_{i}, U^k_{i}), i \in D_c\}$

            Go to line 2 \tcp{continue with next customer}
}

$Sol \leftarrow Sol \backslash \{(L^k_{i}, U^k_{i}), i \in (D^{j'}_{o},..,D^{j-1}_{o})  \}$

Update $\bar{q}_o$

$j \leftarrow j'$, $\lambda \leftarrow delay$

    Go to line 6 \small \tcp{backtrack and retry to insert $D^{j'}_{o}$}
    }
    \uElse{
...
\begin{algorithm}[!ht]
    ...
    $M \hspace{-2mm} \leftarrow \emptyset $ \small \tcp{List of pairs of loading -- unloading tasks for each delivery node}
    ...
    \While{$CL \neq \emptyset$}{
        ...
        \uIf{$T = \emptyset$}{
           ...
           $c \leftarrow $ customer associated with $d$
            ...
            $Sol \leftarrow Sol \backslash \{(L^k_j,U^k_j), j \in D_c \}$
           }
           $CL \leftarrow CL\backslash CL'$
        }
        \uElse{
            $c_{min} \leftarrow \min\left\{ cost \ | \ (L^k_{d}, U^k_{d},cost,k) \in T   \right\} $, $c_{max} \leftarrow \max\left\{ cost \ | \ (L^k_{d}, U^k_{d},cost,k) \in T   \right\} $
...
\begin{table}[ht!]
 ...
 & 112 & 131 & 148 & 8 & 3,953.5 & 24,313.5 & 34.9 & 1,251.9 & 524.6 & 2,961.1 & 4,838,688.0 & 25,301.2 & 13.5 & 1,694.5 & 59.4 & 766.0 & 2,944.6 & 3,151,661.8 \\
    ...
    \vspace{0.1cm}
    {\scriptsize \hspace*{-5mm} $TC$: travel cost, $UQ$: undelivered quantity, $FDD$: first delivery delay, $mFDD$: maximum of first delivery delays, $DUC$: driver underutilization cost, $DOC$: driver overtime cost.}
    \end{minipage}
\end{table}
...
\begin{table}[ht!]
    ...
    %  \cmidrule{3-7}
     &  & \multicolumn{1}{c}{1 min} & \multicolumn{1}{c}{5 min} & \multicolumn{1}{c}{10 min} & \multicolumn{1}{c}{30 min} &\multicolumn{1}{c}{60 min} \\
    \midrule
...
\begin{table}[ht!]
 ...
 & 112 & 131 & 148 & 8 & 3,953.5 & 25,300.6 & 0.0 & 387.0 & 412,296.4 & 26,296.6 & 0.0 & 247.8 & 32.5 & 274,119.6 \\
    ...
    \begin{minipage}{\textwidth}
        \vspace{0.1cm}
        {\scriptsize $TC$: travel cost, $UQ$: undelivered quantity, $FDD$: first delivery delay, $mFDD$: maximum of first delivery delays.}
        \end{minipage}
\end{table}
...
\begin{table}[!ht]
    ...
    \toprule
     ...
     & \multicolumn{1}{c}{$TC$ (min)} & \multicolumn{1}{c}{$FDD$ (min)} & \multicolumn{1}{c}{$mFDD$ (min)} & \multicolumn{1}{c}{$TC$ (min)} & \multicolumn{1}{c}{$FDD$ (min)} & \multicolumn{1}{c}{$mFDD$ (min)}\\
    %  & \multicolumn{1}{c}{(min)} & \multicolumn{1}{c}{(min)}  & \multicolumn{1}{c}{(min)} & \multicolumn{1}{c}{(min)}  & \multicolumn{1}{c}{(min)} & \multicolumn{1}{c}{(min)}  \\
    \midrule
