...
for jour in ticket_date:
    ...
    with open(f"{INSTANCE_FOLDER}/{jour.year}{jour.month}{jour.day}.txt", 'w') as file:
        contents = []
        order_count = 0
        ...
        for i,ord in enumerate(orders_of_day):
            ...
            for j in range(len(ordres)):
                ...
                contents.append(f"{order_count} {ord} {ordre['CUST_NBR']} {ordre['LOCATION ID']} {ordre['SCHED_LOC']} {ordre['SHIPTIME (min)']:.0f} {ordre['QUANTITY']:.2f} {ordre['TRAVEL_MINUTES']} {ordre['UNLOAD_MINUTES']} {pouring_minutes} \n")
        ...
        for i,id in enumerate(list_driver):
            ...
            contents.append(f"{i} {dr.nbr} {dr.rank} {dr.depot_nbr} {dr.factory_nbr} {dr.capacity:.0f} {begin.hour*60 + begin.minute} {end.hour*60 + end.minute} \n")
            # print(end)
            # break
