...
void RechercheLocale::Run(Sol &s) {
	...
	// RelocateStartLoad(s);
	long elapsed = Parameters::GetElapsedTime();
	...
	RunAllFeasible(s);
	elapsed = Parameters::GetElapsedTime();
	if (s.unscheduledCustomers.empty()) {
		bool entree = true;
		while (entree) {
			if (Parameters::GetElapsedTime() - elapsed > 5000) break;
			entree = false;
			if (RelocateStartLoad(s)) {
				entree = true;
				continue;
			}
			if (LoadBackward(s)) {
...
bool RechercheLocale::RelocateStartLoad(Sol &s) {
	...
	while (found) {
		found = false;
		if (s.lateCustomers.empty())
			break;
		if (Parameters::GetElapsedTime() - runtime > 1000)
			break;
...
void RechercheLocale::RunAllFeasible(Sol &s) {
	...
	while (found) {
		...
		for (int i: orderList) {
			...
			if (s.orderLoads[o->orderID].size() == 1)
				continue;
			if (seen[o->orderID] != -1)
				continue;
...
bool RechercheLocale::RemoveAndReschedule(Sol &s) {
	...
	while (found) {
		...
		for (auto id: customerIdList) {
			if (s.unscheduledCustomers.empty())
				break;
			if (Parameters::GetElapsedTime() - runtime > 2000)
				break;
...
bool RechercheLocale::SwapDriver(Sol &s) {
	bool sortie = false;
	keyCustomer = s.keyCustomers;
	availableDriver = s.availableDrivers;
	cout << "SwapDriver" << endl;
	found = true;
	runtime = Parameters::GetElapsedTime();
	while (found) {
		found = false;
		std::vector<int> deliveries(s.GetDeliveryCount());
		std::iota(deliveries.begin(),deliveries.end(),0);
		std::shuffle(deliveries.begin(),deliveries.end(),Parameters::RANDOM_GEN);
		for(int i=0;i<(int)deliveries.size()-1;i++){
			Delivery *del_i = s.GetDelivery(i);
			Driver *d_i = s.GetDriverAssignedTo(del_i);
			if(d_i== nullptr) continue;
			for(int j=i+1;j<(int)deliveries.size();j++){
				Delivery *del_j = s.GetDelivery(j);
				Driver *d_j = s.GetDriverAssignedTo(del_j);
				if(d_j== nullptr) continue;
				if(d_i->id == d_j->id) continue;
				if (Parameters::GetElapsedTime() - runtime > 1000)
					break;
				Sol cur(s);
				cur.UnassignDelivery({del_i,del_j});
				Sol::FixDriver[del_i->delID]=d_j->id;
				Sol::FixDriver[del_j->delID]=d_i->id;
				cur.availableDrivers = {d_i->id,d_j->id};
				cur.keyCustomers = {del_i->custID,del_j->custID};
				CDPSolver::BuildOnSolution(cur,*cur.GetData(),1);
				Sol::FixDriver[del_i->delID]=-1;
				Sol::FixDriver[del_j->delID]=-1;
				if(cur < s){
					s = cur;
					s.keyCustomers = keyCustomer;
					found = true;
...
bool RechercheLocale::SwapLoad(Sol &s, Order *o) {
	...
	for (int i = 0; i < s.GetDeliveryCount(o); i++) {
		...
		for (int j = i + 1; j < s.GetDeliveryCount(o); j++) {
			...
			if (cur.GetDriverAssignedTo(dj) == nullptr)
				break;
