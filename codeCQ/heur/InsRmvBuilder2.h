#ifndef INSRMV_BUILDER2_H
#define INSRMV_BUILDER2_H
...
class InsRmvBuilder2 : public InsRmvBuilder
{
public:

    explicit InsRmvBuilder2(Data &prob)
    {
    ...
    void SetServiceParams(Sol &s, Delivery *n, Driver *d, double demand) override
    {
        ...
        if(n->rank==0){
            _arrival.emplace_back(s.EarlyTW(n) - Parameters::INTRA_ORDER_DELIVERY, insrmv.max_arrival_Time);
        }
        else
        {
            ...
            _arrival.emplace_back(
                    std::min(insrmv.max_arrival_Time, s.EndServiceTime[prec_del_of_cust->id]+val),
                            insrmv.max_arrival_Time);
